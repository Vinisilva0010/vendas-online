    ...
    <section className="w-full bg-base border-b-[8px] border-support py-20 pl-4 md:pl-12 z-20 relative overflow-hidden">
      <div className="max-w-7xl mx-auto pr-4 md:pr-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            <h2
              className="font-title text-4xl md:text-6xl uppercase text-text leading-none"
              style={{ textShadow: "4px 4px 0px var(--color-shock)" }}
            >
              ARTIGOS & GUIAS
            </h2>
            <p className="font-mono text-shock text-sm uppercase tracking-widest mt-2">
              [ ANÁLISES TÁTICAS & DIRETRIZES DE VENDAS ONLINE ]
            </p>
          </div>

          <Link
            href="/blog"
            aria-label="Acessar o diretório completo de artigos e análises de vendas online"
      ...
      {/* CARROSSEL BRUTALISTA */}
      <div className="max-w-7xl mx-auto">
        {posts.length === 0 ? (
          <p className="font-mono text-support text-sm uppercase pr-4 md:pr-12">
            ...
            {posts.map((post) => (
              ...
              >
                ...
                <div className="font-mono text-xs font-bold text-text uppercase flex items-center justify-between w-full border-t-[2px] border-support/30 pt-4 group-hover:text-shock transition-colors relative z-10">
                  ...
                  <span className="text-shock group-hover:translate-x-1 transition-transform">
                    {"->"}
                  </span>
                </div>
              </article>
      ...
      {/* Esconde scrollbar nativa Webkit */}
      <style
